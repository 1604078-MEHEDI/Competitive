...
// tutorial: https://www.youtube.com/watch?v=0IKldsELq9s
#include <bits/stdc++.h>
using namespace std;
#define INF 1<<30
#define endl '\n'
...
ll query(ll l, ll r, ll x, ll y)
{
  ...
  for (x += PN, y += PN; x < y; x >>= 1, y >>= 1) {
    if (x & 1) {
      ret += (upper_bound(fac[x].begin(), fac[x].end(), r)) - (lower_bound(fac[x].begin(), fac[x].end(), l));
      x++;
    }
    if (y & 1) {
      y--;
      ret += (upper_bound(fac[y].begin(), fac[y].end(), r)) - (lower_bound(fac[y].begin(), fac[y].end(), l));
    }
  }
  return ret;
}
