...
void dfs(ll s, ll k)
{
  ...
  range[s].second = timer;
  timer++;
}

void Propagate(ll pos)
{
  ll x = pos * 2;
  seg[x] += seg[pos];
  seg[x + 1] += seg[pos];
...
void update(ll pos, ll L, ll R, ll l, ll r, ll v)
{
  ...
  if (seg[pos] != 0) Propagate(pos);
  ll x = pos * 2;
  ll m = (L + R) / 2;
  update(x, L, m, l, r, v);
  update(x + 1, m + 1, R, l, r, v);
}
...
//*/
  ...
  for (ll i = 0; i < m; i++) {
    ...
    if (t == 1) {
      ...
      ll u;
      scanf("%lld", &u);
      ll ans = Query(1, 1, n, range[u].second);
      ans = (ans * coeff[u] + a[u]);
      printf("%lld\n", ans);
    }
  }
